      ...
      <div className="flex items-center justify-between mb-4 mt-2">
        <div className="relative flex items-center" ref={portfolioMenuRef}>
          ...
          >
            ...
            <ChevronDown className={`w-3 h-3 sm:w-3.5 sm:h-3.5 text-secondary transition-transform ${isPortfolioMenuOpen ? 'rotate-180' : ''}`} />
          </button>
          ...
          {isPortfolioMenuOpen && (
            <div className="absolute top-full left-0 mt-1.5 w-56 bg-surface border border-divider rounded-lg py-1 z-50 shadow-2xl shadow-black/50">
              {portfolios.map(p => (
                ...
                >
                  <div className={`w-3.5 h-3.5 shrink-0 rounded-sm border ${p.id === activePortfolioId ? 'bg-primary border-primary flex items-center justify-center' : 'border-secondary'}`}>
                    {p.id === activePortfolioId && <Check className="w-2.5 h-2.5 text-background" />}
                  </div>
